...
# Model fitting
spp_rcbd_mix_mod <- lmer(yield_kgha ~ krate_kgha*nrate_kgha
                         + (1|rep/krate_kgha), 
                         data = spp_rcbd_dfw )
...
```{r k rate means}
spp_rcbd_mix_means_k <- emmeans(spp_rcbd_mix_mod, 
                                  ~krate_kgha)

spp_rcbd_mix_means_k
```
...
```{r n rate means }
spp_rcbd_mix_means_n <- emmeans(spp_rcbd_mix_mod, 
                                  ~nrate_kgha)

spp_rcbd_mix_means_n
```
It is 209.  
...
Points to notice:  
  ...
  - When comparing means from the interaction, the SE of those means is a combination of the SEs from both Whole Plot and Split Plot errors.  
